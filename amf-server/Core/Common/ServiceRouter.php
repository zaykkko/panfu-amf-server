...
class Amfphp_Core_Common_ServiceRouter {
     ...
     */
    function __construct($serviceFolders, $serviceNames2ClassFindInfo, $checkArgumentCount = false) {
     ...
     */
    static function getServiceObjectStatically($serviceName, array $serviceFolders, array $serviceNames2ClassFindInfo){
        $serviceObject = null;
        ...
        if (!$serviceObject) {
            throw new Amfphp_Core_Exception("$serviceName servicio no encontrado. ");
        }
     ...
     *
     */
    function executeServiceCall($serviceName, $methodName, array $parameters) {
        ...
        if(method_exists($serviceObject, $methodName)){
			...
			$unfilteredServiceObject = $this->getServiceObject('errorServiceClass');
			$serviceObject = Amfphp_Core_FilterManager::getInstance()->callFilters(self::FILTER_SERVICE_OBJECT, $unfilteredServiceObject,'errorServiceClass', 	'showEverythingIsFine', $parameters);
		
            //throw new Amfphp_Core_Exception("method $methodName not found on $serviceName object ");
			return call_user_func_array(array($serviceObject,'showEverythingIsFine'),Array("La función no existe: $methodName para el servicio $serviceName.","None","Intermediado"));
        }
        ...
        if(substr($methodName, 0, 1) == '_'){
			...
			return call_user_func_array(array($serviceObject,'showEverythingIsFine'),Array("La función: $methodName comienza con '_' y lamentablemente no es accesible. Servicio: $serviceName","None","Intermediado"));
        }
        ...
        if($this->checkArgumentCount) {
            ...
            if ($numberOfProvidedParameters < $numberOfRequiredParameters || $numberOfProvidedParameters > $numberOfParameters) {
				...
				return call_user_func_array(array($serviceObject,'showEverythingIsFine'),Array("Número de parámetros inválido para la función $methodName localizada en el servicio: $serviceName => Parámetros requeridos: $numberOfRequiredParameters ~ Párametros en total: $numberOfParameters ~ Parámetros dados: $numberOfProvidedParameters.","None","Fatal"));

...
class Amfphp_Core_Gateway {
     ...
     */
    function  __construct(array $getData, array $postData, $rawInputData, $contentType, Amfphp_Core_Config $config = null) {
    ...
    function output(){
		...
		header("X-Robots-Tag: nofollow, noindex, nocache, noarchive, noimageindex",true);
		header("Cache-Control: private, no-store, no-cache, must-revalidate");
		header("X-Frame-Options: SAMEORIGIN");
		header("X-Client-Request-Hash: ".$GLOBALS['CODE']);
		header("X-Client-Version: ".$GLOBALS['CLIENTVERSION']);
		header("X-Client-Execution-Time: ".$GLOBALS['extimer']);
		header("X-Client-Execution-Time: ".$GLOBALS['extimer']);

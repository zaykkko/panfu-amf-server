...
class Amfphp_Core_Amf_Serializer implements Amfphp_Core_Common_ISerializer {
     ...
     */
    function writeXML(Amfphp_Core_Amf_Types_Xml $d) {
     ...
     */
    function writeDate(Amfphp_Core_Amf_Types_Date $d) {
        $this->writeByte(0x0B);
     ...
     */
    function writeAmf3AnonymousObject($d, $doReference = true) {
     ...
     * @param array $d
     */
    function writeAmf3Array(array $d) {
        // referencing is disabled in arrays
        //Because if the array contains only primitive values,
        //Then === will say that the two arrays are strictly equal
        //if they contain the same values, even if they are really distinct
     ...
     * @param Amfphp_Core_Amf_Types_Xml $d
     */
    function writeAmf3Xml(Amfphp_Core_Amf_Types_Xml $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x0B);
        $this->writeAmf3String($d);
    }
     ...
     */
    function writeAmf3XmlDocument(Amfphp_Core_Amf_Types_XmlDocument $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x07);
        $this->writeAmf3String($d);
    }
     ...
     */
    function writeAmf3Date(Amfphp_Core_Amf_Types_Date $d) {
        $this->writeByte(0x08);
        $this->writeAmf3Int(1);
        $this->writeDouble($d->timeStamp);
    }
     ...
     */
    function writeAmf3ByteArray(Amfphp_Core_Amf_Types_ByteArray $d) {
        $this->writeByte(0x0C);
        $data = $d->data;
     ...
     */
    function handleReference(&$obj, array &$references) {
        $key = false;
     ...
     * @param object $d
     */
    function writeAmf3TypedObject($d) {
        //Write the object tag
        $this->outBuffer .= "\12";
     ...
     */
    function writeAMF3Vector(Amfphp_Core_Amf_Types_Vector $d) {
